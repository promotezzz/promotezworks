...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  useLayoutEffect(() => {
    ...
    const calculateStaticOffsets = () => {
      // Temporarily clear styles to measure static offsets
      const savedStyles = cards.map(c => {
        const trans = c.style.transform;
        const filt = c.style.filter;
        c.style.transform = 'none';
        c.style.filter = 'none';
        return { transform: trans, filter: filt };
      });
      ...
      cards.forEach((c, idx) => {
        c.style.transform = savedStyles[idx].transform;
        c.style.filter = savedStyles[idx].filter;
      });
    };
    ...
    calculateStaticOffsets();
    setupLenis();
    updateCardTransforms();

    window.addEventListener('resize', calculateStaticOffsets);

    return () => {
      window.removeEventListener('resize', calculateStaticOffsets);
